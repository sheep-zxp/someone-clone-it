...
function getOpenAIBaseUrl(): string {
  const fromEnv =
    ...
    process.env.OPENAI_BASE_URL_COMPAT
  const raw = (fromEnv || 'https://api.openai.com/v1').trim()
...
function toOpenAIChatMessages(messages: Message[], systemPrompt: SystemPrompt) {
...
export async function* queryOpenAIWithStreaming({
  ...
  if (!apiKey) {
    yield createAssistantAPIErrorMessage({
      content: `${API_ERROR_MESSAGE_PREFIX}: OPENAI_API_KEY is required when provider=openai`,
  ...
  const chatMessages = toOpenAIChatMessages(messages, systemPrompt)
  const requestBody = {
    model,
  ...
  yield createStreamEvent(messageStartEvent, 0)
  ...
  while (true) {
    ...
    for (const event of events) {
      ...
      try {
        ...
        logForDebugging(`[OpenAI] Failed to parse SSE chunk: ${data}`)
      }
      if (!chunk) continue
